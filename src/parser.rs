...
pub enum Node {
  ...
  ConditionalExpression {operator: Vec<u8>, children: Vec<Node>},
  Null,
}
...
pub fn t_greater_than_or_equal(input: Tokens) -> IResult<Tokens, Token> {
  let fxn = check_token(&|tk| match tk.kind {
...
pub fn t_less_than_or_equal(input: Tokens) -> IResult<Tokens, Token> {
  let fxn = check_token(&|tk| match tk.kind {
...
pub fn comparison_operator(input: Tokens) -> IResult<Tokens, Token> {
  alt((
      t_greater_than,
      t_less_than,
...
pub fn expression(input: Tokens) -> IResult<Tokens, Node> {
   let (input, result) =  alt((boolean, math_expression, function_call, number, string,identifier, conditional_expression))(input)?;
